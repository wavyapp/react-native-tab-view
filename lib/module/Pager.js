...
export default class Pager extends React.Component {
  constructor(...args) {
    ...
    _defineProperty(this, "transitionTo", index => {
      ...
      [cond(not(clockRunning(this.clock)), I18nManager.isRTL ? set(this.initialVelocityForSpring, multiply(-1, this.velocityX, this.springVelocityScale)) : set(this.initialVelocityForSpring, multiply(this.velocityX, this.springVelocityScale))), spring(this.clock, {
        ...state,
    ...
    cond(I18nManager.isRTL ? lessThan(this.gestureX, 0) : greaterThan(this.gestureX, 0),
    ...
    set(this.index, this.nextIndex), set(this.nextIndex, UNSET)])), cond(eq(this.gestureState, State.ACTIVE), [this.maybeCancel, cond(this.isSwiping, NOOP, [
    ...
    round(min(max(0, sub(this.index, cond(greaterThan(this.extrapolatedPosition, 0), I18nManager.isRTL ? DIRECTION_RIGHT : DIRECTION_LEFT, I18nManager.isRTL ? DIRECTION_LEFT : DIRECTION_RIGHT))), sub(this.routesLength, 1))),
    // Index didn't change/changed due to state update
    this.index))]), this.progress]));
    _defineProperty(this, "getTranslateX", memoize((layoutWidth, routesLength, translateX) => multiply(
